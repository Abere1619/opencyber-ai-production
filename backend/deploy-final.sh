...
if curl -s http://localhost:8000/health > /dev/null; then
    ...
    curl -s http://localhost:8000/health | head -2
else
...
if curl -s http://localhost:8080/health > /dev/null; then
    ...
    echo "⚠️  NGINX routing test failed (backend might not be ready yet)"
fi
...
echo "   Stop all:       podman stop opencyber-db opencyber-redis opencyber-backend opencyber-nginx opencyber-grafana"
echo "   Start all:      podman start opencyber-db opencyber-redis opencyber-backend opencyber-nginx opencyber-grafana"
